package com.healthcurrent.data_management.model.test;

public class VitalSignsPatient {

	String vitalSignId;
	String vitalSignLocalId;
	String description;
	String dataSource;
	String dataSourceName;
	String patientId;
	String patientLocalId;
	String last;
	String middle;
	String first;
	String dob;
	
	
	public String getVitalSignLocalId() {
		return vitalSignLocalId;
	}
	public void setVitalSignLocalId(String vitalSignLocalId) {
		this.vitalSignLocalId = vitalSignLocalId;
	}
	public String getDescription() {
		return description;
	}
	public void setDescription(String description) {
		this.description = description;
	}
	public String getVitalSignId() {
		return vitalSignId;
	}
	public void setVitalSignId(String vitalSignId) {
		this.vitalSignId = vitalSignId;
	}
	public String getDataSource() {
		return dataSource;
	}
	public void setDataSource(String dataSource) {
		this.dataSource = dataSource;
	}
	
	public String getDataSourceName() {
		return dataSourceName;
	}
	public void setDataSourceName(String dataSourceName) {
		this.dataSourceName = dataSourceName;
	}
	public String getPatientId() {
		return patientId;
	}
	public void setPatientId(String patientId) {
		this.patientId = patientId;
	}
	public String getPatientLocalId() {
		return patientLocalId;
	}
	public void setPatientLocalId(String patientLocalId) {
		this.patientLocalId = patientLocalId;
	}
	public String getLast() {
		return last;
	}
	public void setLast(String last) {
		this.last = last;
	}
	public String getMiddle() {
		return middle;
	}
	public void setMiddle(String middle) {
		this.middle = middle;
	}
	public String getFirst() {
		return first;
	}
	public void setFirst(String first) {
		this.first = first;
	}
	public String getDob() {
		return dob;
	}
	public void setDob(String dob) {
		this.dob = dob;
	}
	
	
}
